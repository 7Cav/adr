...
exports.up = (pgm) => {
  ...
  pgm.createTable('diff_events', {
    id:            { type: 'serial', primaryKey: true },
    snapshot_id:   { type: 'int', notNull: true, references: '"snapshots"', onDelete: 'CASCADE' },
  ...
  pgm.sql('CREATE INDEX IF NOT EXISTS idx_diff_events_snapshot ON diff_events(snapshot_id)');
  pgm.sql('CREATE INDEX IF NOT EXISTS idx_diff_events_type     ON diff_events(event_type)');
  pgm.sql('CREATE INDEX IF NOT EXISTS idx_diff_events_created  ON diff_events(created_at DESC)');
  pgm.sql('CREATE INDEX IF NOT EXISTS idx_snapshots_fetched    ON snapshots(fetched_at DESC)');
};
