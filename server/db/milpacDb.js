  ...
  for (const u of users) {
    ...
    profiles[String(u.user_id)] = {
      rank: {
        ...
        rankImageUrl: rid && u.rank_image ? rankImageUrl(rid, u.rank_image) : '',
      },
      primary: {
      ...
      records: (recordsByRelation[u.relation_id] ?? []).map(r => ({
        ...
        recordDate:    r.record_date ? new Date(r.record_date * 1000).toISOString().slice(0, 10) : '',
        recordDetails: r.details ?? '',
      })),
      awards: (awardsByRelation[u.relation_id] ?? []).map(a => ({
        awardUid:     String(a.record_id),
        awardName:    a.award_name ?? '',
        awardDate:    a.award_date ? new Date(a.award_date * 1000).toISOString().slice(0, 10) : '',
...
module.exports = { fetchRosterProfiles };
