...
export function UnitFilterBar({ events, unitFilter, onSelect, onClear }) {
  const unitMap = useMemo(() => {
    ...
    for (const e of events) {
      ...
      if (!battalions.has(u.battalion)) {
        battalions.set(u.battalion, { companies: new Set(), platoons: new Map() })
      }
  ...
  const platoons = (bnData && selectedCo && selectedCo !== 'HQ')
    ? [...(bnData.platoons.get(selectedCo) ?? [])].sort((a, b) => Number(a) - Number(b))
    : []
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2.5 space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider mr-1">Unit</span>
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            {companies.map((co) => (
              <UnitChip
                ...
                onClear={selectedCo === co ? () => onClear('company') : undefined}
              />
            ))}
          </div>
        </>
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            {platoons.map((pl) => (
              <UnitChip
                ...
                onClear={selectedPl === pl ? () => onClear('platoon') : undefined}
              />
            ))}
          </div>
        </>
