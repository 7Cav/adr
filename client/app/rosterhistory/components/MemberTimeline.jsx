...
function eventSummary(e) {
  switch (e.event_type) {
    case "PROMOTION":
      return `Promoted ${e.old_value} → ${e.new_value}`;
    case "NEW_MEMBER":
      return "Joined roster";
    case "DISCHARGE":
      return "Discharged";
    case "NEW_RECORD":
      return formatRecordType(e.new_value ?? "");
    case "NEW_AWARD":
      return e.new_value ?? "New award";
                ...
                >
                  <span
                    className={cn(
                      ...
                      EVENT_DOT_COLORS[e.event_type],
                    )}
                  />
                  <div className="min-w-0 flex-1">
                    <span className="font-medium opacity-80 mr-1.5">
                      {EVENT_LABELS[e.event_type]}
                    </span>
                    <span className="opacity-70">{eventSummary(e)}</span>
                    {e.detail && (
                      <p className="opacity-50 mt-0.5 leading-relaxed">
                        {e.detail}
                      </p>
                    )}
                  </div>
