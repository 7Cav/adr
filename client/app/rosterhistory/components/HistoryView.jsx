...
export function HistoryView() {
  ...
  const from = selectedRange
    ? format(subDays(new Date(), selectedRange - 1), "yyyy-MM-dd")
    : null;
  const {
    data: rangeData,
  ...
  const presentRosterTypes = useMemo(() => {
    ...
    for (const e of activeData?.events ?? [])
      if (e.roster_type) s.add(e.roster_type);
    return s;
  }, [activeData]);
  ...
  const matchedProfile = useMemo(() => {
    ...
    const hits = (activeData?.events ?? []).filter((e) =>
  ...
  const heatmapData = useMemo(() => {
    ...
    const cutoff = selectedRange
      ? subDays(new Date(), selectedRange - 1)
      : null;
    ...
    for (const s of summaries) {
      ...
      for (const [type, cnt] of Object.entries(s.counts))
        byBucket[bucketKey].counts[type] =
          (byBucket[bucketKey].counts[type] ?? 0) + cnt;
    }

    return Object.values(byBucket).sort(
      (a, b) => new Date(b.date) - new Date(a.date),
    );
  }, [summaries, selectedRange]);

  const maxCount = useMemo(
    () => Math.max(1, ...heatmapData.map(totalCount)),
    [heatmapData],
  );
      ...
      {heatmapData.length > 0 && (
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide">
            Activity — click a{" "}
            {!selectedRange || selectedRange > 365
              ? "month"
              : selectedRange > 90
                ? "week"
                : "day"}{" "}
          ...
          <div className="flex items-center gap-1.5 mt-2 text-xs text-muted-foreground">
            ...
            {[
              ...
              <span
                key={c}
                className={cn("h-3 w-3 rounded-sm inline-block", c)}
              />
            ))}
            <span>More</span>
          </div>
