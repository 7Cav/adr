...
import { EVENT_COLORS, EVENT_DOT_COLORS, EVENT_LABELS, ALL_EVENT_TYPES, formatRecordType } from '../lib/constants'

export function SummaryBar({ counts, activeFilters, onToggle, recordTypeCounts, excludedRecordTypes = new Set(), onToggleRecordType }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {ALL_EVENT_TYPES.map((type) => {
        ...
        if (type === 'NEW_RECORD' && recordTypeCounts && Object.keys(recordTypeCounts).length > 0) {
          const breakdown = Object.entries(recordTypeCounts).sort((a, b) => b[1] - a[1])
          return (
            <div key={type} className="flex flex-wrap gap-1.5 items-center">
              <button
                onClick={() => onToggle(type)}
                onContextMenu={(e) => { e.preventDefault(); setRecordsExpanded((v) => !v) }}
                className={cn(
              ...
              >
                ...
                <button
                  ...
                  title={recordsExpanded ? 'Collapse breakdown' : 'Expand breakdown'}
                >
                  {recordsExpanded ? '▲' : '▼'}
                </button>
              </button>
              {recordsExpanded && breakdown.map(([rawType, n]) => {
                ...
                return (
                  ...
                  >
                    <span>{n}</span>
                    <span className="opacity-80">{formatRecordType(rawType)}{n !== 1 ? 's' : ''}</span>
                  </button>
                )
              })}
            </div>
