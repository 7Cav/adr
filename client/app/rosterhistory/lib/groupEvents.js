...
export function groupAndSortEvents(events, rankOrder) {
  ...
  const notable = events
    ...
    .sort((a, b) => {
      const orderDiff = (EVENT_SORT_ORDER[a.event_type] ?? 99) - (EVENT_SORT_ORDER[b.event_type] ?? 99)
  ...
  for (const e of events.filter((e) => e.event_type === 'NEW_RECORD')) {
    if (!byPerson.has(e.profile_id)) {
      byPerson.set(e.profile_id, { profileId: e.profile_id, profileName: e.profile_name, records: [] })
    }
    byPerson.get(e.profile_id).records.push(e)
  }

  const recordGroups = Array.from(byPerson.values()).sort((a, b) => {
    const rankDiff = rankVal(a.records[0]?.rank_short ?? '') - rankVal(b.records[0]?.rank_short ?? '')
    if (rankDiff !== 0) return rankDiff
